...
async function sendPushNotification(expoPushToken: string, title: string, body: string) {
  await fetch('https://exp.host/--/api/v2/push/send', {
    ...
    body: JSON.stringify({
      ...
      title,
      body,
      data: { screen: 'home' },  // navigazione al tap
    }),
  })
}
```
...
```bash
# Build development client (per test con librerie native)
eas build --profile development --platform all

# Build preview (APK Android + IPA iOS per TestFlight)
...
# Submit direttamente all'App Store / Play Store
...
```bash
# Configura OTA in eas.json → profile production ha già autoIncrement
...
// 3. Certificate pinning per API critiche (se necessario)
// npx expo install expo-crypto expo-modules-core

// 4. Jailbreak/root detection (se l'app tratta dati finanziari/sanitari)
import * as Device from 'expo-device'
if (!Device.isDevice) {
